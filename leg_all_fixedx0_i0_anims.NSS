/*
    ...
    Script:			Built in script that had a bug in it.
	Version:		1.00
	Plugin Version: 1.6
	Author:			Marshall Vyper
	Parameters:		N/A
	
	Change Log:		10/17/2011 - 1.00 MV - Initial Release
...
//:://////////////////////////////////////////////////
// ChazM 4/25/06 Added Serenity to AnimActionPlayRandomTalkAnimation()
// ChazM 8/23/06 modified AnimActionRest() to simply return false.  Added dwarf to the list of civilized races in CheckIsCivilized()
// ChazM 9/5/06 Added GetRandomQuasiFriend() and modified AnimActionFindFriend() to use it.
// ChazM 9/13/06 Modified AnimActionFindFriend() - Fix for bug that was cutting social animations off early
...
#include "leg_all_fixedx0_i0_walkway"
...
// As above but will look at creatures >=75 Reputation
object GetRandomQuasiFriend(float fMaxDistance);
...
object GetRandomQuasiFriend(float fMaxDistance)
{
	...
	if (GetReputation(OBJECT_SELF, oFriend) < 75)
		return (OBJECT_INVALID);
...
void AnimActionPlayRandomUncivilized()
{
    ...
    if (nRoll != 5) {
		...
		//PrettyDebug("X0_i0_anims: AnimActionPlayRandomUncivilized() " + GetName(OBJECT_SELF) + ": doing ActionRandomWalk" );
...
//    } else
   {
        ...
        if (!GetIsObjectValid(oFriend))
			oFriend = GetRandomQuasiFriend(fMaxDistance);
...
{
	// ChazM 8/23/06
	// Rests were commented out.  I'm speculating this was because wandering monsters
	// use ambient animations, and monsters healing might make some encounters too difficult.
	// 	Returning false always since we won't actually do anything about being wounded.
/*
...
void AnimActionPlayRandomTalkAnimation(int nHDiff)
{
	...
	int iSerenity = GetLocalInt(OBJECT_SELF, "Serenity");
